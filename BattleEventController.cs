...
namespace Yukar.Battle
{
    ...
    public class BattleEventController : BattleEventControllerBase
    {
        ...
        // Retain the original location of monsters deleted while alive with BTL_APPEAR using UniqueID→moveTargetPos
            ...
            MapEngine mapEngine)
        {
            ...
            // Record party information at the start of battle (so that it is not overwritten when regenerating the layout)
        ...
        internal void RestoreParty()
        {
            ...
            // When term() is called twice, owner is null
            if (owner == null)
                return;
        ...
        internal void update()
        {
            ...
            // The event character during battle (runner's mapChr borrowed from dummyChrs / mapScene of common event for battle) is
        ...
        internal new void Draw()
        {
            ...
            // メニューより前面のフリーレイアウト描画
            // Free layout drawing in front of the menu
            if (battle.battleState < BattleState.FinishFadeIn)
        ...
        public override void GetCharacterScreenPos(MapCharacter chr, out int x, out int y, EffectPosType pos = EffectPosType.Ground, Vector3? offset = null)
        {
            ...
            if (viewer != null)
            {
                ...
                // (Countermeasures for an issue where the playback frame advances extra and the camera goes into the ground while damage is being displayed)
        ...
        // Is the target selection cursor displayed (during selection operation)?
        // 決定後は selector に選択が残るが、それは実対象確定用なので選択中とはみなさない
        // After the selection, the selection remains in the selector, but it is not considered to be selected because it is used to confirm the actual object.
