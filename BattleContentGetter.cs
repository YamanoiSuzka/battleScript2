...
namespace Yukar.Battle
{
    ...
    {
        /// <summary>
        /// 控えのデータを BattlePlayerData として参照するために一時的に保存しておくDictionary
        /// Dictionary that temporarily stores backup data for reference as BattlePlayerData
        /// </summary>
        public static Dictionary<Common.GameData.Hero, BattlePlayerData> sReserveTemporaryDataCache = new Dictionary<Common.GameData.Hero, BattlePlayerData>();
            ...
            GameContentCreator owner, RenderContent result, ref GameContentParser.StatusUpDown refStatusUpDown)
        {
            ...
            if (battleItems != null)
            {
                if (battleItems.GetChoicesData().Count == 0)
                {
                    battle.UpdateItemSelectWindowDrawer();
                }
            }
            ...
            // 控えを含める場合
            // If you include a copy
            if (gameMain.data.party.PartyType == LayoutProperties.LayoutNode.PartyTypes.PartyAndReserve)
                players = players.Concat(gameMain.data.party.Reserves.Select(x =>
                {
                    if (sReserveTemporaryDataCache.ContainsKey(x))
                    {
                        return sReserveTemporaryDataCache[x];
                    }

                    var stat = battle.createPlayerData(x, false);
                    battle.UpdateBattleStatusData(stat);
                    sReserveTemporaryDataCache[x] = stat;
                    return stat;
                }
                )).ToList();
            #endregion
            ...
            switch (content.type)
            {
                ...
                case GameContentParser.ContentType.PARTYNAME_A:
                    {
                        ...
                        if (heros != null && heros.Count > partyIndex)
                        {
                            var partyName = players[partyIndex].Name;
                            result.ElementType = SpecialTextRenderer.SpecialTexts.Party;
                            return partyName;
                        }
                        else
                ...
                case GameContentParser.ContentType.PARTYNAME:
                    {
                        ...
                        if (heros != null && heros.Count > index)
                        {
                            var partyName = players[index].Name;
                            result.ElementType = SpecialTextRenderer.SpecialTexts.Party;
                            return partyName;
                        }
                        else if (gameContent.LayoutState is LayoutStateBattleMember)
                        {
                            result.ElementType = SpecialTextRenderer.SpecialTexts.Party;
                            return catalog.getGameSettings().glossary.change_member_party_to_reserve;
                        }
                        else
                        {
                            result.ElementType = SpecialTextRenderer.SpecialTexts.Party;
                ...
                case GameContentParser.ContentType.PARTYSTATUS_A:
                    {
                        ...
                        if (heros.Count > index)
                        {
                            ...
                            return GetStatusImpl(hero, gameContent, content, owner, index);
                        }
                        return "";
                    }
                ...
                case GameContentParser.ContentType.CURRENTPARTYGRAPHIC:
                    ...
                    break;
                case GameContentParser.ContentType.CURRENTPARTYSTATUS_A:
                    {
                        var heros = players;
                        var partyIndex = heros.FindIndex(x => x.player == gameContent?.Hero);
                        if (heros.Count > partyIndex && partyIndex >= 0)
                        {
                            var hero = heros[partyIndex];
                            var heroStatus = GetStatusImpl(hero, gameContent, content, owner, partyIndex);
                            result.ElementType = SpecialTextRenderer.SpecialTexts.Party;
                            return heroStatus;
                        }
                    }
                    break;
                case GameContentParser.ContentType.CURRENTPARTYCONDITION_A:
                    {
                        var heros = players;
                        var partyIndex = heros.FindIndex(x => x.player == gameContent?.Hero);
                        if (heros.Count > partyIndex && partyIndex >= 0)
                    ...
                    break;
                case GameContentParser.ContentType.CURRENTPARTYCONDITION:
                    {
                        var heros = players;
                        var partyIndex = heros.FindIndex(x => x.player == gameContent?.Hero);
                        if (heros.Count > partyIndex && partyIndex >= 0)
                    ...
                    break;
                case GameContentParser.ContentType.CURRENTPARTYCONDITIONICON_A:
                    {
                        var heros = players;
                        var partyIndex = heros.FindIndex(x => x.player == gameContent?.Hero);
                        if (heros.Count > partyIndex && partyIndex >= 0)
                        {
                            ...
                            if (hero != null)
                            {
                                if (owner.SetConditionIconThumbnail(result, hero.conditionInfoDic, content.attrs[0]))
                                {
                                    result.ElementType = SpecialTextRenderer.SpecialTexts.Party;
                                }
                            }
                    ...
                    break;
                case GameContentParser.ContentType.CURRENTPARTYCONDITIONICON:
                    {
                        var heros = players;
                        var partyIndex = heros.FindIndex(x => x.player == gameContent?.Hero);
                        if (heros.Count > partyIndex && partyIndex >= 0)
                        {
                            ...
                            if (hero != null)
                            {
                                if (owner.SetConditionIconThumbnail(result, hero.conditionInfoDic))
                                {
                                    result.ElementType = SpecialTextRenderer.SpecialTexts.Party;
                                }
                            }
                ...
                case GameContentParser.ContentType.ITEMDES:
                    {
                        if (items.Count > index)
                        {
                            result.ElementType = SpecialTextRenderer.SpecialTexts.Item;
                            return items[index].text2;
                        }
                        else
                        {
                            return "";
                ...
                case GameContentParser.ContentType.ITEMMAXNUM:
                    {
                        if (items.Count > index)
                        {
                            result.ElementType = SpecialTextRenderer.SpecialTexts.Item;
                            return items[index].item?.maxNum.ToString() ?? "";
                        }
                        else
                        {
                            return "";
...
#if false
                //NOTE(sb_matsu):currentシリーズをここでやるなら…のサンプル
                //              :CurrentItemContentGetterのCURRENTITEMNAMEでやっても同じ
                // The same applies when you do it with CURRENTITEMNAME in :CurrentItemContentGetter
                case GameContentParser.ContentType.CURRENTITEMNAME:
                    {
                        if (gameContent.ItemStack != null)
                        {
                            result.ElementType = SpecialTextRenderer.SpecialTexts.Item;
        ...
        private string GetStatusImpl(BattlePlayerData hero, AbstractRenderObject.GameContent gameContent, GameContentParser.CachedResult.Content content, GameContentCreator owner, int index)
        {
            if (content.growthFactor)
            {
                return owner.GetGrowthFactor(hero, content.attrs[0]);
...
//          }
        ...
        //  var max = inBattlePlayer.battleStatusData.statusValue.GetStatus(info.guId);
            var max = inBattlePlayer.GetStatus(inCatalog.getGameSettings(), info.guId);// #24812 battleStatusData.statusValueは加工済みのため、こちらを使ってみる [sb_matsu] / #24812 battleStatusData.statusValue has already been processed, so try using this [sb_matsu]
        ...
        public override List<NItem> GetBattleUsableItems()
        {
            var list = new List<NItem>();

            if (BattleSequenceManagerBase.Get() is BattleSequenceManager battle)
            {
                var battleItems = battle?.battleViewer?.itemSelectWindowDrawer;
                var items = battleItems?.GetChoicesData();
                if (items != null)
                {
                    list.AddRange(items.Select(item => item.item));
                }
            }
