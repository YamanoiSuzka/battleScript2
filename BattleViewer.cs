...
namespace Yukar.Battle
{
    ...
    public class BattleViewer
    {
		...
		protected List<EffectDrawerBase> defeatEffectDrawers = new List<EffectDrawerBase>();
        public bool IsEffectAllowShowDamage { get { return playerEffectDrawer.isEndPlayingOrOverDamageTime &&
                    monsterEffectDrawer.isEndPlayingOrOverDamageTime &&
                    battleCharacterEffectDrawerDic.Values.All(ed => ed.isEndPlayingOrOverDamageTime); } }
        public bool IsEffectEndPlay { get { return playerEffectDrawer.isEndPlaying && monsterEffectDrawer.isEndPlaying &&
                    battleCharacterEffectDrawerDic.Values.All(ed => ed.isEndPlaying); } }
        virtual public bool IsEndMotion(BattleCharacterBase chr = null) { return true; }

		protected Dictionary<BattleCharacterBase, EffectDrawerBase> battleCharacterEffectDrawerDic = new Dictionary<BattleCharacterBase, EffectDrawerBase>();
        ...
        public float displayMessageFrameCount;// displayMessageText が画面に表示され続けている累計フレーム数（60FPS基準） / Cumulative number of frames in which displayMessageText continues to be displayed on the screen (60FPS standard)
        TweenVector2 messageWindowPosition;

        protected Common.Resource.Texture damageNumberImageId;
        protected Common.Resource.Texture damageMissedImageId;
        ...
        internal Dictionary<BattleCharacterBase, EffectDrawerBase> defeatEffectDrawTargetList;
        ...
        public BattleViewer(GameMain owner)
        {
            ...
            defeatEffectDrawTargetList = new Dictionary<BattleCharacterBase, EffectDrawerBase>();
        ...
        public virtual void LoadResourceData(Catalog catalog, Rom.GameSettings gameSettings)
        {
            ...
            spriteRes = catalog.getItemFromGuid(gameSettings.GraphicBattleMissed) as Resource.Texture;
            if (spriteRes == null)
            {
                damageMissedImageId = null;
            }
            else
            {
                damageMissedImageId = Graphics.LoadImageDiv(spriteRes.path, 1, 1);
            }
        ...
        internal virtual void ReleaseResourceData()
        {
            ...
            defeatEffectDrawers.Clear();

            foreach (var effectDrawer in battleCharacterEffectDrawerDic.Values.Distinct())
            {
                effectDrawer.finalize();
            }

			battleCharacterEffectDrawerDic.Clear();
        ...
        internal virtual void Draw(List<BattlePlayerData> playerData, List<BattleEnemyData> enemyMonsterData)
        {
			...
			foreach (var monster in enemyMonsterData.Where(monster => monster.imageAlpha > 0).OrderBy(monster => monster.IsSelect).ThenByDescending(monster => monster.arrangmentType))
            {
                ...
                else if (defeatEffectDrawTargetList.ContainsKey(monster))
                {
                    // エフェクトを受けてる時はエフェクトターゲット色を適用
                    // Apply effect target color when receiving effect
                    color = defeatEffectDrawTargetList[monster].getNowTargetColor();
                }
                else
                {
        ...
        internal virtual void SetBattlePlayerEffect(Guid guid, BattleCharacterBase target)
        {
        ...
        {
            var defeatEffectDrawer = defeatEffectDrawers.Find(effect => effect.isSameRom(guid));
            ...
            if (!defeatEffectDrawTargetList.ContainsKey(target))
                defeatEffectDrawTargetList.Add(target, defeatEffectDrawer);
        }

		internal void SetBattleCharacterEffect(Guid guid, BattleCharacterBase target)
        {
            var effectDrawer = battleCharacterEffectDrawerDic.Values.FirstOrDefault(x => x.isSameRom(guid));
            if (effectDrawer == null)
            {
                ...
                // If it is shared, it is not reused and a new instance is created by passing null (protecting the replay of other keys).
            ...
            //EffectDrawerBase effectDrawer = battleCharacterEffectDrawerDic.ContainsKey(target) ? battleCharacterEffectDrawerDic[target] : null;
			//SetBattleEffect(ref effectDrawer, guid, target);
			//battleCharacterEffectDrawerDic[target] = effectDrawer;
        ...
        {
            if (effectDrawer == null || !effectDrawer.isSameRom(guid))
            {
