...
namespace Yukar.Battle
{
    ...
    public class BattleViewer3D : BattleViewer
    {
        ...
        public class BattleUI
        {
            ...
            /// </summary>
            public class LayoutDrawerController
            {
...
//#if DUNGEON
//                           || battleSequenceManager?.battleState == BattleState.ResultInit
//#endif
                               )
                            {
                        ...
                        else
                        {
                            ...
                            // When hidden, return the variable assigned to the item under mouse hover to -1
                ...
                {
                    gameMain.data.party.PartyType = partyType;
                ...
                internal void LoadLayout(GameMain owner, Catalog catalog, Rom.LayoutProperties.LayoutNode layout, Rom.LayoutProperties.LayoutNode.UsageInGame usageInGame)
                {
					...
					switch (layout.Usage)
					{
						...
						case Rom.LayoutProperties.LayoutNode.UsageInGame.BattleItem:
                            ...
                            break;
						case Rom.LayoutProperties.LayoutNode.UsageInGame.Member:
							filteringTag = layout.FilteringTag;
							break;
						default:
							break;
                ...
                internal void finalize()
                {
                    ...
                    // The layout will be closed when the battle ends, so return the variable assigned to the item under mouse hover to -1.
                    AbstractLayoutState.ResetHoveringVariable(drawer, gameMain?.data?.system);
                ...
                internal void OnDecide()
                {
                    AbstractLayoutState.SetVariableValueImpl(AbstractLayoutState.SetVariableSituation.DECIDE,
                        drawer, gameMain.data.system, drawer.GetSelectIndex());
                }

                internal void OnCancel()
                {
                    AbstractLayoutState.SetVariableValueImpl(AbstractLayoutState.SetVariableSituation.CANCEL,
                        drawer, gameMain.data.system, drawer.GetSelectIndex());
                }
            }
        ...
        private Rom.RenderSettings originalRenderSettings;
        ...
        internal override void BattleStart(List<BattlePlayerData> playerData, List<BattleEnemyData> enemyMonsterData)
        {
            ...
            if (game.data.IsBattleCameraEnabled(Common.GameData.SystemData.BATTLE_CAMERA_SITUATION_START, catalog))
            {
            ...
            {
                // オフセット座標設定
                // Offset coordinate settings
                if (mapDrawer.mapRom != null)
                {
                    int x = mapDrawer.mapRom.Width / 2;
                    int y = mapDrawer.mapRom.Height / 2;

                    m_VrCameraData.SetOutOffsetPos(
                        mapDrawer.mapRom.Width * 0.5f,
                        mapDrawer.getAdjustedHeight(x, y),
                        mapDrawer.mapRom.Height * 0.5f
                        );
                }

                // VR情報更新
                // VR information update
        ...
        /// <summary>
        /// エフェクト対象のActorが存在するかチェックするためのオーバーライド
        /// Override to check if the effect target actor exists
        /// </summary>
        /// <param name="guid"></param>
        /// <param name="target"></param>
        internal override void SetBattlePlayerEffect(Guid guid, BattleCharacterBase target)
        {
            if (searchFromActors(target) == null)
                return;

            base.SetBattlePlayerEffect(guid, target);
        }
        ...
        private void drawEffect()
        {
            ...
            foreach (var defeatEffectDrawer in defeatEffectDrawers)
            {
                if (!defeatEffectDrawer.isEndPlaying)
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (target.Value == defeatEffectDrawer)
                            {
                                defeatEffectDrawer.draw((int)target.Key.EffectPosition.X, (int)target.Key.EffectPosition.Y, false);
                                var actor = searchFromActors(target.Key);
                                defeatEffectDrawer.drawFor3D(skillSource?.mapChr, actor?.mapChr, defeatEffectDrawTargetList.Count(x => x.Value == defeatEffectDrawer), rot.Y);
            ...
            var centerScreenEffectList = new List<Rom.RomItem>();

            foreach (var target in effectDrawTargetMonsterList)
            {
				if (battleCharacterEffectDrawerDic.ContainsKey(target))
				{
                    var battleCharacterEffectDrawer = battleCharacterEffectDrawerDic[target];

                    if (battleCharacterEffectDrawer.isEndPlaying)
                    {
                        continue;
                    }

                    if (battleCharacterEffectDrawer.origPos == Common.Resource.NSprite.OrigPos.ORIG_SCREEN)
                    {
                        if (!centerScreenEffectList.Contains(battleCharacterEffectDrawer.effect))
                        {
                            // 画面全体を対象としたエフェクトの場合、ターゲット色だけを反映して、エフェクト自体は画面全体に出す
                            // In the case of an effect that targets the entire screen, only the target color is reflected, and the effect itself is displayed on the entire screen.
                            battleCharacterEffectDrawer.draw(Graphics.ScreenWidth / 2, Graphics.ScreenHeight / 2);

                            centerScreenEffectList.Add(battleCharacterEffectDrawer.effect);
                        }
                    }
                    else
                    {
						battleCharacterEffectDrawer.drawFlash();
				...
				if (battleCharacterEffectDrawerDic.ContainsKey(target))
				{
					...
					else
					{
						...
						if (targetLayoutNode != null)
						{
							const int HEIGHT_OFFSET = 80;
							var pos = targetLayoutNode.GetOriginPosition();
							battleCharacterEffectDrawer.draw((int)pos.X, (int)pos.Y, false);
                            // 3D座標から求める場合の例
                            // Example of finding from 3D coordinates
                            //var near = Util.UnProject2World(p, v, new SharpKmyMath.Vector3(pos.X, pos.Y + HEIGHT_OFFSET, -1), Graphics.ScreenWidth, Graphics.ScreenHeight);
                            //var far = Util.UnProject2World(p, v, new SharpKmyMath.Vector3(pos.X, pos.Y + HEIGHT_OFFSET, 1), Graphics.ScreenWidth, Graphics.ScreenHeight);
							...
							if (actor != null)
							{
								...
								var pos3dBack = actor.mapChr.pos;
								actor.mapChr.pos = new Vector3(pos.X * BattleUI.PARTICLE_SCALE, -(pos.Y + HEIGHT_OFFSET) * BattleUI.PARTICLE_SCALE, 0);
								if (battleCharacterEffectDrawer is EffectDrawer3D e3d)
									e3d.setDisplayId(Util.BATTLEPARTICLEDISPLAYID);
								if (battleCharacterEffectDrawer.drawFor3D(skillSource?.mapChr, actor.mapChr, effectDrawTargetPlayerList.Count, rot.Y))
									actor?.queueActorState(BattleActor.ActorStateType.DAMAGE_START);
								actor.mapChr.pos = pos3dBack;
							}
						}
		...
		private void drawEnemyInfo(List<BattleEnemyData> enemyMonsterData)
        {
            ...
            foreach (var monster in enemyMonsterData)
            {
                ...
                else if (effectDrawTargetMonsterList.Contains(monster))
                {
                    ...
                    if (battleCharacterEffectDrawerDic.ContainsKey(monster))
                    {
                        effect = battleCharacterEffectDrawerDic[monster];
                    }
                }
                ...
                else if (defeatEffectDrawTargetList.ContainsKey(monster))
                {
                    effect = defeatEffectDrawTargetList[monster];
                }
                else if (displayWindow == WindowType.CommandTargetMonsterListWindow)
                {
                    if (monster.IsSelect)
        ...
        private void drawPlayerEffect(BattlePlayerData player)
        {
            ...
            else if (defeatEffectDrawTargetList.ContainsKey(player))
            {
                effect = defeatEffectDrawTargetList[player];
            }
			else if (battleCharacterEffectDrawerDic.ContainsKey(player))
			{
				effect = battleCharacterEffectDrawerDic[player];
			}
        ...
        internal BattleActor AddEnemyMember(BattleEnemyData data, int index, bool useCustomLayout = false)
        {
            // 1スタートになっている
            // 1 has started
            index--;
            ...
            if (useCustomLayout)
            {
                ...
                actor.mapChr.setPosition(neutralPos);
            }
            else
            {
        ...
        internal BattleActor AddPartyMember(BattlePlayerData data, Vector3[] playerLayouts, int index = -1)
        {
            // 空きを探す
            ...
            // インデックスの指定があり、なおかつ空じゃない場合
            // If an index is specified and it is not empty
            else if (friends[empty] != null)
            {
                // 指定番号以降を後ろにずらす
                // Shift the specified number backwards
                for (int i = friends.Count - 1; i > empty; i--)
                {
                    friends[i] = friends[i - 1];
                }
            }

            if (empty < 0 || friends.Count <= empty)
                return null;
                    ...
                    .OrderByDescending(info => info.targetCharacter.EffectPriority))
                {
                    ...
                    foreach (var characterInfo in info.textInfo)
                    {
                        ...
                        else
                        {
                            ...
                            Graphics.DrawStringSoloColor(info.font, characterInfo.c.ToString(), position, color);
                            basePosition.X += Graphics.MeasureString(info.font, characterInfo.c.ToString()).X;
                        }
                    }
        ...
        internal BattleActor searchFromActors(BattleCharacterBase battleCharacterBase)
        {
            ...
            // 見つからない場合、ユニークIDで探す
            // If not found, search by unique ID
            if (battleCharacterBase is BattleEnemyData)
            {
                foreach (var mapChr in enemies)
                {
                    if (mapChr == null)
                    ...
                    if (mapChr.source.UniqueID == battleCharacterBase.UniqueID)
                        return mapChr;
                }
            }
            return null;
        ...
        /// Get the coordinates of the person acting (active cast).
        /// </summary>
        ...
        /// Obtain the coordinates of the action target cast (center coordinates in case of multiple targets).
        /// </summary>
        /// <param name="fallback">行動先ターゲット・敵のいずれも居ない場合に返す座標</param>
        /// <param name="fallback">Coordinates returned when there is no target or enemy</param>
        ...
        void UpdateBattleActors(List<BattleActor> inBattleActors, float inAngleY, bool inIsFriend)
        {
            foreach (var actor in inBattleActors)
            {
                ...
                // 敵の死亡エフェクト
                // Enemy death effect
                if (fadeoutEnemyList.Count > 0 && source.imageAlpha > 0 && !defeatEffectDrawTargetList.ContainsKey(source) &&
        ...
        internal override void Update(List<BattlePlayerData> playerData, List<BattleEnemyData> enemyMonsterData)
        {
            ...
            if (game.data.IsBattleCameraEnabled(Common.GameData.SystemData.BATTLE_CAMERA_SITUATION_RESULT, catalog) &&
            ...
            for (int i = 0; i < playerData.Count; i++)
            {
                    ...
                    owner.battleState <= BattleState.SortBattleActions)
                {
                    ...
                    // Auto battle (IsUseBattleAI) does not require command selection and the AI makes an immediate decision, so
                    // 選択中の一歩前進/後退の演技(コマンド決定→即後退に見える)を対象から除外する
                    // Exclude the currently selected step forward/backward performance (command decision → looks like immediate retreat) from the target.
                ...
                switch (self.selectedBattleCommandType)
                {
                    ...
                    case BattleCommandType.ForceCritical:
                        if (start)
                        {
                            ...
                            if (game.data.IsBattleCameraEnabled(Common.GameData.SystemData.BATTLE_CAMERA_SITUATION_ATTACK, catalog))
                            {
                            ...
                            if (target == null)
                                target = actor;// 見つからなくてもバトルが進行するようにする / Make the battle proceed even if you are not found.
                    ...
                    case BattleCommandType.SameSkillEffect:
                        if (start)
                        {
                            ...
                            //if (actor.frontDir > 0)
                            {
                                ...
                                // Guid.Empty = Standard / Camera.NONE_CAMERA_GUID = None (Skip) / Other = User Battle Camera
                                ...
                                if (!skipActivationPresentation && !skipSkillCamera &&
                                    game.data.IsBattleCameraEnabled(Common.GameData.SystemData.BATTLE_CAMERA_SITUATION_SKILL, catalog))
                                {
                                    ...
                                    // Specified user battle camera (fallback to standard skill camera if not found)
                    ...
                    case BattleCommandType.Item:
                        if (start)
                        {
                            ...
                            if (actor.frontDir > 0)
                            {
                                if (game.data.IsBattleCameraEnabled(Common.GameData.SystemData.BATTLE_CAMERA_SITUATION_ITEM, catalog))
                                {
                    ...
                    case BattleCommandType.Change:
                        if (start)
                        {
                            ...
                            //if (actor.frontDir > 0)
                            {
                                if (game.data.IsBattleCameraEnabled(Common.GameData.SystemData.BATTLE_CAMERA_SITUATION_CHANGE, catalog))
                                {
                    ...
                    case BattleCommandType.Enter:
                        if (start)
                        {
                            ...
                            //if (actor.frontDir > 0)
                            {
                                if (game.data.IsBattleCameraEnabled(Common.GameData.SystemData.BATTLE_CAMERA_SITUATION_ENTER, catalog))
                                {
                    ...
                    case BattleCommandType.Leave:
                        if (start)
                        {
                            ...
                            //if (actor.frontDir > 0)
                            {
                                if (game.data.IsBattleCameraEnabled(Common.GameData.SystemData.BATTLE_CAMERA_SITUATION_LEAVE, catalog))
                                {
                    ...
                    case BattleCommandType.PlayerEscape:
                        //if (!start)
                        //{
                        //    // friends は BattlePlayerMax 分の固定長で、不在のメンバーの枠には null が入っている。
                        // // friends has a fixed length equal to BattlePlayerMax, and null is placed in the frame for absent members.
                        //    // 行動不能(麻痺等)も KO ステートなので、先頭から順に KO が続くと null の枠に到達する。
                        // // Inability to take action (paralysis, etc.) is also a KO state, so if KO continues from the beginning, the null frame will be reached.
                        //    friends.FirstOrDefault(x => x != null && x.getActorState() != BattleActor.ActorStateType.KO)
                        //        ?.queueActorState(BattleActor.ActorStateType.ESCAPE_FAILED);
                        //}
                        break;
        ...
        internal void createCameraMatrix(out SharpKmyMath.Matrix4 p, out SharpKmyMath.Matrix4 v/*, Rom.ThirdPersonCameraSettings camera*/, float asp, bool advanceTime = true)
        {
            ...
            if (SharpKmyVr.Func.IsReady())
            {
                ...
                // Matrix calculation
                {
                    ...
                    p = SharpKmyMath.Matrix4.translate(shakeValue.x, shakeValue.y, 0) *
                        SharpKmyMath.Matrix4.perspectiveFOV(MathHelper.ToRadians(fovy), asp, camManager.m_nearClip, farclip);
                    if (mapDrawer.sky != null)
                    {
                        mapDrawer.sky.setProj(SharpKmyMath.Matrix4.perspectiveFOV(MathHelper.ToRadians(fovy), asp, 1, 1000));
                    }
                }
            }
            else
            {
                if (camManager.ntpCamera != null && owner.IsDrawingBattleScene)
                {
                    ...
                    if (advanceTime)
                    {
                        ...
                        if (camManager.isPlayAnim == false)
                        {
                            ...
                            //owner.battleEvents.RefreshCameraTargetPos();
                            camManager.ntpCamera = null;
                        }
                    }
                    else
                ...
                {
                    owner.battleEvents.SetLookAtTarget(null, mCamLookAtTarget);
                    owner.battleEvents.CreateCameraMatrix(out p, out v, farclip);
                    //v = SharpKmyMath.Matrix4.lookat(lookat, target, vecUp);
                    //v = SharpKmyMath.Matrix4.inverse(v);
        ...
        /// <summary>
        /// VRカメラデータをセットアップ
        /// Set up VR camera data
        /// </summary>
        /// <param name="bUpdateInfo"></param>
        public override void SetupVrCameraData(bool bUpdateInfo)
        {
            ...
            // 箱庭視点時のスケール
            // Scale from miniature garden perspective
        ...
        internal void Show()
        {
            ...
            // バトル終了時にもとに戻すために保存しておく
            // Save it to restore it at the end of the battle
            originalRenderSettings = Rom.RomItem.Clone<Rom.RenderSettings>(mapDrawer.renderSettings);

            // 全アクターのモーションをサブグラフィックとの同期のためリセットする
            // Reset all actor motions to synchronize with subgraphics
            foreach (var actor in friends)
            {
                if (actor != null)
                {
                    actor.mapChr.ResetMotion();
                }
            }
        }
        ...
        internal void Hide()
        {
            ...
            // バトル画面のレンダリング設定を元に戻す
            // Restore the battle screen rendering settings
            if (originalRenderSettings != null)
                mapDrawer.setRenderSettings(originalRenderSettings);
            originalRenderSettings = null;

            // マップ画面のレンダリング設定をGameViewに反映させる
            // Reflect the map screen rendering settings in GameView
