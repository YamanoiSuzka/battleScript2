...
namespace Yukar.Battle
{
    ...
    public class ResultViewer
    {
		...
		internal void SetResultData(BattlePlayerData[] partyPlayer, int exp, int money, Rom.NItem[] items, Dictionary<Guid, int> itemDictonary, Common.GameData.Party party = null)
        {
            var partyPlayerDic = new Dictionary<Common.GameData.Hero, BattlePlayerData>();

			foreach (var item in partyPlayer)
			{
                partyPlayerDic.Add(item.Hero, item);
			}

			if (party != null)
            {
                var totalList = party.Players;

                totalList.AddRange(party.Reserves);

				foreach (var item in totalList)
                {
                    if (!partyPlayerDic.ContainsKey(item))
                    {
                        partyPlayerDic.Add(item, null);
					}
                }
            }
            ...
            foreach (var kv in partyPlayerDic)
            {
				var hero = kv.Key;
				var player = kv.Value;

				var addExp = exp * (100 + hero.conditionAddExpRateForResult) / 100;

                if (!partyPlayer.Contains(player))
                {
					// 控え
					// reserve
					addExp = Common.Rom.GameSettings.CalcExp(addExp, gameSettings.ReserveExpFormulaWords);
				}
				...
				data.hero = hero;
				data.player = player;
                data.expRate = 100 + hero.conditionAddExpRateForResult;

                data.castLevelUpData.levelUpExpList = hero.rom.levelUpExpList;
                data.castLevelUpData.startLevel = hero.level;
                data.castLevelUpData.maxLevel = hero.maxLevel;
                data.castLevelUpData.addExp = addExp;
                data.castLevelUpData.baseExp =
                data.castLevelUpData.totalExp = hero.exp;
                data.castLevelUpData.upLevel = 0;
                data.castLevelUpData.preLevel = -1;

                var jobCast = hero.jobCast;
                ...
                data.resultStatus.Name = player?.Name ?? hero.rom.Name;
                data.resultStatus.CurrentLevel = hero.level;
                data.resultStatus.NextLevel = hero.level;
            ...
            switch (resultState)
            {
                ...
                case ResultState.CheckLevelUp:
                    foreach (var index in levelUpIndices.ToArray())
                    {
                        ...
                        var player = characterLevelUpData[index].hero;
        ...
        internal List<CharacterLevelUpData> CheckSkillOver()
        {
            ...
            {
                var currentSkillCount = characterLevelUp.hero.skills.Count;
